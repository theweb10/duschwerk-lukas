...
const INITIAL_STATE = {
  ...
  corrections: [],        // Letzte Auto-Korrekturen
  submitted: false,
};
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'GO_TO_STEP': {
      ...
      // Zurückspringen: immer erlaubt
      if (target < state.step) {
        return { ...state, step: target, corrections: [] };
      }
      // Vorwärtsspringen: alle vorherigen Schritte müssen complete sein
...
export function useConfiguratorState() {
  ...
  // Berechnete verfügbare Optionen
  const options = useMemo(() => {
    const { serie, einbausituation, tuersystem, hoehe } = state.config;
    return {
      einbausituationen: getVerfuegbareEinbausituationen(serie),
      tuersysteme: getVerfuegbareTuersysteme(einbausituation, serie),
      glastypen: getVerfuegbareGlastypen(serie, tuersystem),
      glasstaerken: getVerfuegbareGlasstaerken(serie, hoehe),
      profilfarben: getVerfuegbareProfilfarben(serie),
      rahmentypen: getVerfuegbareRahmentypen(serie, tuersystem),
