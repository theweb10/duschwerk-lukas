...
export default function ContactForm() {
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-5">
      ...
      {/* Subject */}
      <Field label="Betreff" id="subject" required error={errors.subject}>
        <input
          id="subject" name="subject" type="text"
          value={form.subject} onChange={handleChange}
          className={`input-apple ${errors.subject ? 'input-apple-error' : ''}`}
          placeholder="z.B. Anfrage Walk-in-Dusche"
        />
      </Field>

      {/* Message */}
      <Field label="Nachricht" id="message" required error={errors.message}>
        <textarea
          id="message" name="message" rows={5}
          value={form.message} onChange={handleChange}
          className={`input-apple resize-none ${errors.message ? 'input-apple-error' : ''}`}
